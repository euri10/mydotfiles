...
LOCAL_CONFIG_PATH = os.path.join(HOME, ".ncmpcpp/.lastfm-mpd-cli.rc")
...
def mpd_get_current():
    mpc = subprocess.Popen(["ncmpcpp", "--now-playing"], shell=False, stdout=subprocess.PIPE)
    mpc.wait()
...
#    name = track[1].strip()
    artist = re.match('\(\d+:\d+\) (.+) - (.+)',result[0]).group(1)
    name = re.match('\(\d+:\d+\) (.+) - (.+)',result[0]).group(2)
    return (artist, name)
